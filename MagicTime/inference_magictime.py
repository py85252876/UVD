    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    if args.human:
        ...
        for prompt_idx, (prompt, n_prompt, random_seed) in enumerate(zip(prompts, n_prompts, random_seeds)):
            ...
            samples.append(output.videos)
            total_predx_0.append(output.predx_0)
            total_predx_t.append(output.predx_t)
            # prompt = "-".join((prompt.replace("/", "").split(" ")[:10]))
            save_videos_grid(output.videos, f"{savedir}/sample/{sample_idx}.mp4")
            print(f"save to {savedir}/sample/{sample_idx}.mp4")
            sample_idx += 1
        total_predx_0 = torch.stack(total_predx_0,dim=0)
        total_predx_t = torch.stack(total_predx_t,dim=0)
        print(total_predx_0.shape)
